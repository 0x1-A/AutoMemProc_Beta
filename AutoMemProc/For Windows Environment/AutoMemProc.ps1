...
function Find-Python3Executable {
    $specifiedPath = "C:\Windows\py.exe"

    if (Test-Path $specifiedPath) {
        $versionOutput = & $specifiedPath --version 2>&1
        if ($versionOutput -match "Python (\d+\.\d+)") {
            $version = $matches[1]
            if ([double]$version -ge 3.0) {
                $confirmed = $false
                while (-not $confirmed) {
                Write-Host "`nPython v3 found at $specifiedPath with Version: $version. Confirm using this path (y/n): " -ForegroundColor darkyellow -NoNewLine
                ...
                Write-Host "Python v3 is a primary requirement to run vol3. The provided Python executable at $specifiedPath is of version $version. Please provide a valid Python v3 path." -ForegroundColor red
            }
        }
    } else {
        Write-Host "Python v3 is a primary requirement to run vol3. The provided path $specifiedPath does not exist" -ForegroundColor red
    }

    while ($true) {
        $manualPath = Read-Host "Please enter the path to Python v3 executable (e.g., C:\Path\To\python3.exe)" -ForegroundColor darkyellow -NoNewLine
        if (Test-Path $manualPath) {
            $versionOutput = & $manualPath --version 2>&1
            if ($versionOutput -match "Python (\d+\.\d+)") {
                $version = $matches[1]
                if ([double]$version -ge 3.0) {
                    $confirmed = $false
                    while (-not $confirmed) {
                        $confirmation = Read-Host "`nPython v3 found at $manualPath with Version: $version. Confirm using this path (y/n):" -ForegroundColor Green -NoNewLine
                        switch ($confirmation) {
                            "y" {
                                $confirmed = $true
                                return $manualPath
                    ...
                    Write-Host "The provided Python executable is of version $version. Please provide a valid Python v3 path." -ForegroundColor red
                }
            }
        } else {
            Write-Host "The provided path not valid." -ForegroundColor red
        }
    }
}

# Example of using the function:
$selectedPython3Path = Find-Python3Executable
Write-Host "`nSelected Python v3 executable path: $selectedPython3Path" -ForegroundColor Green


function VolToolPath {
    param (
        [string]$prompt = "↬ Enter the path to the Volatility Tool (e.g., \path\to\vol.py): ",
...
if (Test-Path -LiteralPath $configFilePath) {
    ...
    if ($savedPath -and (Test-Path -LiteralPath $savedPath -PathType Leaf)) {
        while ($true) { 
            Write-Host ""
            Write-Host "↬ Found saved Volatility path: $savedPath. Do you want to use this path (y/n): " -ForegroundColor Darkyellow -NoNewLine
...
Write-Host ""
   Write-Host "Python path: ($selectedPython3Path)" -ForegroundColor Green
Write-Host ""
   Write-Host "Votaility 3 Path: ($memoryImagePath)" -ForegroundColor Green
Write-Host ""
...
Set-Location -Path $dumpDir
            ...
            Write-Host ""
            $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath windows.dumpfiles.DumpFiles --pid $processID
...
if (Confirm-Action) { 
      $outputFileName = "Info_output.txt"
        $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath -r pretty windows.info.Info
       $outputFile = Join-Path -Path (Get-Item $memoryImagePath).DirectoryName -ChildPath $outputFileName
                    if (Test-Path -Path $outputFile) 
                         {
                                 $outputFile = Ask-Overwrite -filePath $outputFile
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.psscan.PsScan --pid $processID"
            if ($grepTerms) {
               $command += " | grep -i '$grepTerms'"
            }
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.psscan.PsScan"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.pslist.PsList --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.pslist.PsList"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.pstree.PsTree --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.pstree.PsTree"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.cmdline.CmdLine --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.cmdline.CmdLine"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.dlllist.DllList--pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.dlllist.DllList"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.getsids.GetSIDs --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.getsids.GetSIDs"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.privileges.Privs --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.privileges.Privs"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.handles.Handles --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.handles.Handles"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.envars.Envars --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.envars.Envars"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.verinfo.VerInfo"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.malfind.Malfind --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.malfind.Malfind"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
12 {
                                ...
                                $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath windows.skeleton_key_check.Skeleton_Key_Check
                                $outputFile = Join-Path -Path (Get-Item $memoryImagePath).DirectoryName -ChildPath $outputFileName
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.svcscan.SvcScan --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.svcscan.SvcScan"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.sessions.Sessions--pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.sessions.Sessions"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.joblinks.JobLinks"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $mainProcess = {
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath -r pretty windows.memmap.Memmap"
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.vadinfo.VadInfo --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.vadinfo.VadInfo"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
        ...
        Write-Host ""
                             ...
                             3 {
                                   if (Confirm-Action) { 
                                      ...
                                      $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath -r pretty windows.virtmap.VirtMap
                                $outputFile = Join-Path -Path (Get-Item $memoryImagePath).DirectoryName -ChildPath $outputFileName
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.vadwalk.VadWalk --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.vadwalk.VadWalk"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    if ($inputProcessID) {
        foreach ($processID in $processIDsArray) {
            ...
            $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.ldrmodules.LdrModules --pid $processID"
            if ($grepTerms) {
                $command += " | grep -i '$grepTerms'"
            }
            $imageinfoOutput = Invoke-Expression $command
        ...
        $outputFileName = "VadWalk_output.txt"
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.ldrmodules.LdrModules"
        if ($grepTerms) {
            $command += " | grep -i '$grepTerms'"
        }
        $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.modscan.ModScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.modules.Modules"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.driverscan.DriverScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.driverirp.DriverIrp"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.filescan.FileScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.symlinkscan.SymlinkScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.mutantscan.MutantScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.callbacks.Callbacks"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.ssdt.SSDT"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
11 {
    ...
    $mainProcess = {
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.devicetree.DeviceTree"
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.netstat"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.netscan.NetScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.registry.hivescan.HiveScan"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.registry.hivelist.HiveList"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
    ...
    Write-Host ""
                         ...
                         $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath windows.registry.printkey.PrintKey --key $registryKey
                                 }       
                                 else {                   
                                 $outputFileName = "PrintKey_output_nokey.txt"
                                 $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath windows.registry.printkey.PrintKey                     
                                 }
                                 $outputFile = Join-Path -Path (Get-Item $memoryImagePath).DirectoryName -ChildPath $outputFileName
                                 if (Test-Path -Path $outputFile) {
                                 $outputFile = Ask-Overwrite -filePath $outputFile
...
if (Confirm-Action) { 
    ...
    Write-Host ""
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.registry.certificates.Certificates"
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.getservicesids.GetServiceSIDs"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.registry.userassist.UserAssist"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.hashdump.Hashdump"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
    ...
    Write-Host ""
                        ...
                        8 {
                                     if (Confirm-Action) { 
                                      ...
                                      $imageinfoOutput = & $selectedPython3Path $volatilityPath -f $memoryImagePath -r pretty windows.lsadump.Lsadump
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.crashinfo.Crashinfo"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
if (Confirm-Action) { 
    ...
    $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.cachedump.Cachedump"
    if ($grepTerms) {
        $command += " | grep -i '$grepTerms'"
    }
    $imageinfoOutput = Invoke-Expression $command
...
1 { 
    ...
    $mainProcess = {
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.mbrscan.MBRScan"
...
1{ 
    ...
    $mainProcess = {
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.poolscanner.PoolScanner"
...
2 { 
    ...
    $mainProcess = {
        ...
        $command = "$selectedPython3Path $volatilityPath -f $memoryImagePath windows.bigpools.BigPools"
